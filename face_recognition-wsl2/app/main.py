...
import face_recognition, os, shutil, json, numpy as np, time

app = FastAPI(title="FaceRecognition Minimal API")
...
async def register_face(name: str = Form(...), file: UploadFile = File(...)):
    ...
    try:
        ...
        embeddings_db[name] = encodings[0].tolist()
        save_embeddings()
...
async def recognize_face(
    ...
    try:
        ...
        for name, db_emb in embeddings_db.items():
            dist = np.linalg.norm(np.array(db_emb) - np.array(query_embedding))
            if dist < min_dist:
                min_dist = float(dist)
                identity = name
